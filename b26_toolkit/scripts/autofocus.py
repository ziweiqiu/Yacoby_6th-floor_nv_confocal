...
Autofocus: Takes images at different piezo voltages and uses a heuristic to figure out the point at which the objective
    ...
    _DEFAULT_SETTINGS = [
        Parameter('save_images', True, bool, 'save image taken at each voltage'),
        ...
        Parameter('center_on_current_location', True, bool, 'Check to use current galvo location rather than center point in take_image'),
        Parameter('galvo_return_to_initial', True, bool, 'Check to return galvo location to initial value (before calling autofocus)'),
    ...
    def _plot(self, axes_list, data = None, colorbar = None):
        ...
        # axis_image, axis_focus = axes_list
        ...
        if current_image is not None:
            plot_fluorescence_new(current_image, extent, axis_image, colorbar = colorbar)
...
#                axis_focus.hold(False)
        ...
        # axis_focus.set_ylabel(self.ylabel)
...
class AutoFocusDAQ(AutoFocusGeneric):
    ...
    def __init__(self, scripts, instruments = None, name = None, settings = None, log_function = None, data_path = None):
        ...
        """
        # print('LOADING')
        self.scan_label = 'objective z voltage [V]'
        try:
            Script.__init__(self, name, settings, instruments, scripts, log_function= log_function, data_path = data_path)
        except:
            import sys
    ...
    def _step_piezo(self, voltage, wait_time):
        ...
        z_piezo = self.instruments['daq']['instance']
        # set the voltage on the piezo
        # z_piezo.voltage = float(voltage)
        z_piezo.set_analog_voltages({self.settings['DAQ_channel_Z']: voltage})
    ...
    def _get_galvo_location(self):
        ...
        Returns: list with two floats, which give the x and y position of the galvo mirror
        """
        galvo_position = self.daq_in.get_analog_voltages(
            [self.settings['DAQ_channels']['x_ao_channel'], self.settings['DAQ_channels']['y_ao_channel']])

        # galvo_position = self.scripts['take_image'].instruments['NI6353']['instance'].get_analog_out_voltages([
    ...
    def _function(self):
        ...
        if self.settings['use_current_z_axis_position']:
            ...
            zv = z_piezo.get_analog_voltages([self.settings['DAQ_channel_Z']])
            self.settings['z_axis_center_position'] = zv[0]
