...
from b26_toolkit.instruments import NI6353, LISE607RTPulseBlaster, Pulse, AgilentMicrowaveGenerator, R8SMicrowaveGenerator
...
for a given experiment
    ...
    _DEFAULT_SETTINGS = [
        ...
        Parameter('MAX_AVERAGES_PER_SCAN', 100000, int, 'the max number of loops per point allowed at one time'),
        Parameter('mw_switch', [
            ...
            Parameter('gating', 'mw_iq', ['mw_switch', 'mw_iq','otherwise'],'determines if mw pulses are carved out by mw-switch or by i and q channels of mw source '),
        ...
        Parameter('min_pulse_dur', 10, [10, 15, 50], 'Minimum allowed pulse duration (ns)')
    ]
    _INSTRUMENTS = {'daq': NI6353, 'PB': LISE607RTPulseBlaster}
    _SCRIPTS = {'find_nv': FindNV, 'esr': ESR_RnS, 'optimize_z': optimize}
    ...
    def __init__(self, instruments, scripts, name=None, settings=None, log_function=None, data_path=None):
        ...
        if self.settings['MAX_AVERAGES_PER_SCAN'] > 100000:
            self.MAX_AVERAGES_PER_SCAN = 100000 # 1E5, the max number of loops per point allowed at one time (true max is ~4E6 since
                                 #pulseblaster stores this value in 22 bits in its register
        else:
            self.MAX_AVERAGES_PER_SCAN = self.settings['MAX_AVERAGES_PER_SCAN']
    ...
    def _calc_progress(self, index):
        ...
        if self.current_averages >= self.MAX_AVERAGES_PER_SCAN:
            progress = (self.current_averages + (1.0 - progress_inner) * self.MAX_AVERAGES_PER_SCAN) / self.num_averages
        else:
    ...
    def _function(self, in_data=None):
        ...
        # print('PulsedExperimentBaseScript starts')
        """
        ...
        # not so important here commented it out -- Ziwei 2/7/2019
        ...
        self.instruments['PB']['instance'].update({'min_pulse_dur': self.settings['min_pulse_dur']})
        ...
        self.count_sqrd_sum = np.repeat([np.append(signal, norms)], len(self.pulse_sequences), axis=0) # will be updated

        self.data = in_data
        self.data['tau'] = np.array(self.tau_list)
        self.data['counts'] = deepcopy(self.count_data)
        self.data['counts_err'] = np.repeat([np.append(signal, norms)], len(self.pulse_sequences), axis=0)
        ...
        print("Averaging over {0} blocks of 1e5".format(num_1E5_avg_pb_programs))

        average_loop = 0

        # Turn on the laser for a few seconds to stabilize the power right before the sequence starts
        ...
        for average_loop in range(int(num_1E5_avg_pb_programs)):
            ...
            if self._abort:
                ...
                print('aborted pulseblaster script during loop')
                break

            # ER 20181028
            # for now, disable this ESR tracking capability
        ...
        #     print('start running sweep')
        #     print(self.pulse_sequences)
        #     print(MAX_AVERAGES_PER_SCAN)
        #     print(num_daq_reads)
            self._run_sweep(self.pulse_sequences, self.MAX_AVERAGES_PER_SCAN, num_daq_reads, self.tau_list, avg_num = average_loop + 1)

        if remainder != 0 and not self._abort:
            self.current_averages = self.num_averages
            self._run_sweep(self.pulse_sequences, remainder, num_daq_reads, self.tau_list, avg_num = int(num_1E5_avg_pb_programs) + 1)
        ...
        if (len(self.data['counts_err'][0]) == 1) and not self._abort:
            self.data['counts_err'] = np.array([item for sublist in self.data['counts_err'] for item in sublist])
        ...
        return (average_loop + 1) * self.MAX_AVERAGES_PER_SCAN
...
#        else:
        ...
        if 'counts' in data.keys():
          ...
          #  print('plotting with tau values: ', data['tau'])
            plot_1d_simple_timetrace_ns(axes_list[0], data['tau'], [data['counts']])
            current_pulse = ' (tau = ' + str(self.data['tau'][self.sequence_index]) + 'ns)'
            # print(current_pulse)
            plot_pulses(axes_list[1], self.pulse_sequences[self.sequence_index], pulse_tag = current_pulse)
...
#        else:
        ...
        axis1 = axes_list[0] # lower subplot for plotting the data
        if not counts == []:
            update_success = update_1d_simple(axis1, x_data, [counts])
            # print('counts_list: ', [counts])
            # print('axis.lines: ', axis1.lines)
            # if not update_success:
            #     plot_1d_simple_timetrace_ns(axes_list[0], self.data['tau'], [self.data['counts']])
        axis2 = axes_list[1] # upper subplot for plotting the sequence
        current_pulse = ' (tau = ' + str(self.data['tau'][self.sequence_index])  + 'ns)'
        # print(current_pulse)
        update_pulse_plot(axis2, self.pulse_sequences[self.sequence_index], pulse_tag = current_pulse)
        # update_pulse_plot(axis2, self.pulse_sequences[self.sequence_index])

        # self.current_rand_index
    ...
    def _run_sweep(self, pulse_sequences, num_loops_sweep, num_daq_reads, tau_list, avg_num = 0, verbose=False):
     ...
     #   self.data['init_fluor'] = 0.
        ...
        for index, sequence in enumerate(pulse_sequences):
            ...
            self.count_sqrd_sum[rand_index] = self.count_sqrd_sum[rand_index] + np.array(counts_to_check) ** 2  # unit: kcps^2
            # counts_to_check maybe a list
            self.data['counts_err'][rand_index] = self._cal_err(self.count_sqrd_sum[rand_index], self.data['counts'][rand_index], avg_num)
    ...
    def _run_single_sequence(self, pulse_sequence, num_loops, num_daq_reads):
        ...
        '''
        # print('start _run_single_sequence')
        ...
        if num_daq_reads != 0:
            # print('start setup_gated_counter')
    ...
    def _cal_err(self, count_sqrd_sum, count_mean, avg_num):
        """
        ...
        result = []
        for i in range(len(count_mean)):
            sigma = np.sqrt((count_sqrd_sum[i] - avg_num * (count_mean[i] ** 2)) / (avg_num*(avg_num-1)))
            result.append(sigma)
    ...
    def _get_overlapping_pulses(self, pulse_sequence, verbose=True):
        ...
        if self.settings['mw_switch']['no_iq_overlap']:
            ...
            #                                                                                'microwave_q'])
            overlapping_pulses = LISE607RTPulseBlaster.find_overlapping_pulses(pulse_sequence,
                                                                         combine_channels=['microwave_i',
                                                                                           'microwave_q'])
        else:
            # overlapping_pulses = B26PulseBlaster.find_overlapping_pulses(pulse_sequence)
    ...
    def _get_commands_that_are_too_short(self, pulse_sequence, verbose=True):
        ...
        min_pulse_dur = self.settings['min_pulse_dur']
        ...
        short_pulses = [command for command in pb_commands if command.duration < min_pulse_dur]
        # verbose = True

        if verbose and short_pulses:
            print('Minimum pulse duration is ' + str(min_pulse_dur) +' ns')
    ...
    def _has_pulses_with_bad_start_time(self, pulse_sequence, verbose=False):
        ...
        for pulse in pulse_sequence:
            ...
            if 0 < pulse.start_time < 1:
                if verbose:
                    print('pulse.start_time is', pulse.start_time)
            ...
            if np.mod(pulse.start_time, 1.0e9/(1.0e6*self.instruments['PB']['instance'].settings['clock_speed'])) != 0:
                print('type II bad start time problem')
                print('pulse.start_time is', pulse.start_time, 'ns')
                print('clock speed is', self.instruments['PB']['instance'].settings['clock_speed'], 'MHz')
                print('mod by', 1.0e9/(1.0e6*self.instruments['PB']['instance'].settings['clock_speed']))
                return True
            if np.mod(pulse.start_time + pulse.duration, 1.0e9/(1.0e6*self.instruments['PB']['instance'].settings['clock_speed'])) != 0:
                print('type III bad start time problem')
                print('pulse.start_time + pulse.duration is', pulse.start_time + pulse.duration, 'ns')
                print('clock speed is', self.instruments['PB']['instance'].settings['clock_speed'], 'MHz')
                print('mod by', 1.0e9 / (1.0e6 * self.instruments['PB']['instance'].settings['clock_speed']))
                return True
    ...
    def _is_bad_pulse_sequence(self, pulse_sequence, verbose=False):
        ...
        if self._get_overlapping_pulses(pulse_sequence, verbose=verbose):
            print('invalid reason: _get_overlapping_pulses')
            return True
        elif self._get_commands_that_are_too_short(pulse_sequence, verbose=verbose):
            print('invalid reason: _get_commands_that_are_too_short')
            return True
        # elif self._has_pulses_with_bad_start_time(pulse_sequence, verbose=verbose): # ignore this problem for now ZQ 2/27/2019
        #     print('invalid reason: _has_pulses_with_bad_start_time')
        #     return True
        elif self._compiles_to_too_many_commands_for_pb(pulse_sequence, verbose=verbose):
            print('invalid reason: _compiles_to_too_many_commands_for_pb')
            return True
        else:
            return False
        ...
        for pulse_sequence in pulse_sequences:
            ...
            if gating == 'mw_iq':
                for pulse in pulse_sequence:
                    ...
                    if pulse.channel_id in ['microwave_i_II', 'microwave_q_II']:
                        mw_switch_pulses_II.append(Pulse('microwave_switch_IV', pulse.start_time - mw_switch_time, pulse.duration + 2 * mw_switch_time))

                # add the mw switch pulses to the pulse sequences
                pulse_sequence.extend(mw_switch_pulses)
                pulse_sequence.extend(mw_switch_pulses_II)
                ...
                pulse_sequence = self._combine_pulses(pulse_sequence, channel_id='microwave_switch_IV', overlap_window= 2 * mw_switch_time)
    ...
    def create_pulse_sequences(self, logging=True, verbose = False):
        """
        ...
        if logging:
            if invalid_tau_list:
                print("The pulse sequences corresponding to the following tau's were *invalid*, thus will not be "
                ...
                print('All generated pulse sequences are valid. No tau times will be skipped in this experiment.')
                self.log("All generated pulse sequences are valid. No tau times will be skipped in this experiment.")

            print('{:d} out of {:d} different tau times have passed validation'.format(len(valid_tau_list), len(tau_list)))
            self.log("{:d} out of {:d} different tau times have passed validation".format(len(valid_tau_list), len(tau_list)))
...
# class PulsedExperimentBaseScript_old_fromB26(Script):
#     """
# This class is a base class that should be inherited by all classes that utilize the pulseblaster for experiments. The
# _function part of this class takes care of high-level interaction with the pulseblaster for experiment control and optionally
# the daq for reading counter input (usually from the APD). It also provides all of the functionality needed to run a
