...
class T1SingleInit(PulsedExperimentBaseScript):
    """
    ...
    --> Last edited by Ziwei Qiu 6/20/2019

    """
    _DEFAULT_SETTINGS = [
        Parameter('tau_times', [
            Parameter('min_time', 15, float, 'minimum tau'),
            Parameter('max_time', 500000, float, 'maximum tau'),
            Parameter('time_step', 1000.,float,'time step increment of tau (in ns)')
        ]),
        Parameter('read_out', [
        ...
        Parameter('MAX_AVERAGES_PER_SCAN', 10000, int, 'the max number of loops per point allowed at one time')
    ]
    ...
    def _function(self):
        ...
        self.avg_block_number = super(T1SingleInit, self)._function()
    ...
    def _create_pulse_sequences(self):
        ...
        # delay_mw_readout = self.settings['read_out']['delay_mw_readout'] # this is not used though
        ...
        for tau in tau_list:
            ...
            pulse_sequence.append(Pulse('laser', laser_off_time + tau, nv_reset_time))
            pulse_sequence.append(Pulse('apd_readout', laser_off_time + tau + delay_readout, meas_time))
            pulse_sequence.append(Pulse('apd_switch', 0, laser_off_time + tau + nv_reset_time))
            # pulse_sequence = \
    ...
    def _plot(self, axislist, data = None):
        ...
        if 'fits' in data.keys() and data['fits'] is not None:
            ...
            # axislist[0].plot(tau, fit_exp_decay(tau, *fits), lw=3)
            axislist[0].set_title('T1 decay of ms = 0 population \n Ref fluor: {:0.1f}kcps')

            axislist[0].set_title(
                'T1 decay of ms = 0 population, T1 = (p = 1) = {:2.1f} ns \n Ref fluor: {:0.1f}kcps'.format(fits[1], mean_fluor))
            ...
            if 0 not in self.data['counts'][:, 0]:
                ...
                axislist[0].set_ylabel('Normalized Fluorescence')
                axislist[0].set_title(
                    'T1 decay of ms = 0 population \n Ref fluor: {:0.1f}kcps'.format(mean_fluor))
            else:
                ...
                axislist[0].set_title('(final plot) T1 decay of ms = 0 population')

        else:
            super(T1SingleInit, self)._plot(axislist)
            axislist[0].set_title('(initial plot) T1 decay of ms = 0 population')
            axislist[0].legend(labels=('Ref Fluorescence', 'T1 Data'), fontsize=8)
    ...
    def _update_plot(self, axislist):
        ...
        if len(axislist[0].lines) == 0:
            self._plot(axislist)
            return
        super(T1SingleInit, self)._update_plot(axislist)
...
# The following three T1 scripts are from B26.
# class T1_B26(PulsedExperimentBaseScript):  # ER 10.21.2017
#     """
...
# class T1SingleInit_B26(PulsedExperimentBaseScript): # ER 5.25.2017
#     """
# This script takes a T1 by measuring the decay of the ms = 0 population,into +/-1. To avoid needing a pi pulse we only do this once on ms = 0
#     """
#     _DEFAULT_SETTINGS = [
...
#             super(T1SingleInit, self)._plot(axislist)
#             axislist[0].set_title('T1 decay of ms = 0 population')
# class T1_double_init_many_NVs_B26(Script):
#     _DEFAULT_SETTINGS = [
