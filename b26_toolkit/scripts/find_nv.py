...
Known issues:
    ...
    _DEFAULT_SETTINGS = [
        ...
        Parameter('sweep_range', .35, float, 'voltage range to sweep over to find a max'),
        Parameter('num_points', 61, int, 'number of points to sweep in the sweep range'),
        Parameter('nv_size', 21, int, 'TEMP: size of nv in pixels - need to be refined!! needs to be odd number!!!'),
        Parameter('min_mass', 80, int, 'TEMP: brightness of nv - need to be refined!!'),
    ...
    def _function(self):
       ...
       # print((self.scripts['take_image'].instruments))
        if self.settings['center_on_current_location']:
            ...
            daq_pt = self.scripts['take_image'].instruments['NI6353']['instance'].get_analog_voltages([self.scripts['take_image'].settings['DAQ_channels']['x_ao_channel'], self.scripts['take_image'].settings['DAQ_channels']['y_ao_channel']])
        ...
        # print('before take image')
        self.scripts['take_image'].run()

        # print('after take image')
    ...
    def _plot(self, axes_list, data = None, colorbar = None):
        ...
        if self._current_subscript_stage['current_subscript'] == self.scripts['take_image']:
            self.scripts['take_image']._plot(axes_list, colorbar = colorbar)
        else:
