...
class PulseBlaster(Instrument):
    ...
    _DEFAULT_SETTINGS = Parameter([
        ...
        Parameter('output_2', [
            ...
            Parameter('status', True, bool, 'True if voltage is high to the microwave p trigger, false otherwise'),
    ...
    def __init__(self, name=None, settings=None, verbose = False):
        try:
        ...
        if verbose:
            print('Pulse Blaster is connected, using SpinAPI library version: ', self.pb.pb_get_version())
        # print('Status bin: ',self.pb.pb_read_status())
        # print('Status message: ', self.pb.pb_status_message())

        super(PulseBlaster, self).__init__(name, settings)
        # self.update(self._DEFAULT_SETTINGS) # added by ZQ
    ...
    def prepare_function_calls(self):
        ...
        self.pb.pb_status_message.restype = (ctypes.c_char_p)
    ...
    def update(self, settings):
        ...
        # print('Successfully initialize the pulseblaster on pb_init() command.')
        # Need to close other programs (such as MATLAB) that might control the pulse blaster.
       # self.pb.pb_reset()
        ...
        # start = self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits() | 0xE00000),
        #                                self.PB_INSTRUCTIONS['CONTINUE'], ctypes.c_int(0), ctypes.c_double(5.0e8))
        # address = self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits() | 0xE00000),
        #                                  self.PB_INSTRUCTIONS['BRANCH'],
        #                                  ctypes.c_int(start), ctypes.c_double(5.0e8))


        # (ZQ 20190513) To address channel number > 20, remove the three 1's in the beginning. Checked it still works.
        start = self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits()),
                                       self.PB_INSTRUCTIONS['CONTINUE'], ctypes.c_int(0), ctypes.c_double(5.0e8))
        address = self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits()),
                                         self.PB_INSTRUCTIONS['BRANCH'],
                                         ctypes.c_int(start), ctypes.c_double(5.0e8))



        # print('20190513ZQ: self.settings2bits() | 0xE00000):' , self.settings2bits() | 0xE00000)

        # start = self.pb.pb_inst_pbonly(ctypes.c_uint32(0xFFFFFF),
        #                                self.PB_INSTRUCTIONS['CONTINUE'], ctypes.c_int(0), ctypes.c_double(5.0e8))
        # address = self.pb.pb_inst_pbonly(ctypes.c_uint32(0xFFFFFF),
        #                                  self.PB_INSTRUCTIONS['BRANCH'],
        #                                  ctypes.c_int(start), ctypes.c_double(5.0e8))

        # self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits() | 0xE00000), self.PB_INSTRUCTIONS['BRANCH'],
        #                        ctypes.c_int(0), ctypes.c_double(100))
        # print(bin(self.settings2bits()| 0xE00000))
        ...
        # start = self.pb.pb_inst_pbonly(ctypes.c_uint32((self.settings2bits())),
        #                                self.PB_INSTRUCTIONS['CONTINUE'], ctypes.c_int(0), ctypes.c_double(5.0e8))
        # address = self.pb.pb_inst_pbonly(ctypes.c_uint32(self.settings2bits()), self.PB_INSTRUCTIONS['BRANCH'],
        #                                  ctypes.c_int(start), ctypes.c_double(5.0e8))
        # print(bin(self.settings2bits()))
        ...
        # print('Status bin: ', self.pb.pb_read_status())
        # print('Status message: ', self.pb.pb_status_message())
        # self.pb.pb_stop() # comment this to keep output static
        ...
        # print('ZQ20181215: pb.update is called')

    def settings2bits(self):
        #COMMENT_ME
        # print('ZQ20181215: pb.settings2bits is called')
        bits = 0
    ...
    def program_pb(self, pulse_collection, num_loops=1):
        ...
        for command in pb_commands:
            if command.duration < self.settings['min_pulse_dur']:
                raise RuntimeError('Detected command with duration <' + self.settings['min_pulse_dur'] + 'ns.')
...
#     _DEFAULT_SETTINGS = Parameter([
#         Parameter('apd_readout', [
#             Parameter('channel', 5, int, 'channel to which the daq is connected to'),
#             Parameter('status', False, bool, 'True if voltage is high to the daq, false otherwise'),
...
#             Parameter('status', False, bool, 'True if voltage is high to the laser, false otherwise'),
#             Parameter('delay_time', 455, float, 'delay time between pulse sending time and laser switch on [ns] (measured as 470ns)')
#         ]),
#         Parameter('laser_yellow', [
#             Parameter('channel', 6, int, 'channel to which laser is connected'),
#             Parameter('status', False, bool, 'True if voltage is high to the laser, false otherwise'),
#             Parameter('delay_time', 555, float,
#                       'delay time between pulse sending time and laser switch on [ns] (measured as 470ns)')
#         ]),
#         Parameter('laser_red', [
#             Parameter('channel', 8, int, 'channel to which laser is connected'),
#             Parameter('status', False, bool, 'True if voltage is high to the laser, false otherwise'),
#             Parameter('delay_time', 425, float,
#                       'delay time between pulse sending time and laser switch on [ns] (measured as 470ns)')
#         ]),
#         Parameter('microwave_switch', [
#             Parameter('channel', 0, int, 'channel to which the microwave switch is connected to'),
#             Parameter('status', False, bool, 'True if voltage is high to the microwave switch, false otherwise'),
...
#         ]),
#         Parameter('aux_sw', [
#             Parameter('channel', 9, int, 'trigger sequence channel - pulse at the beginning of pulse sequence'),
#             Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
#             Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
#         ]),
#         Parameter('clock_speed', 500, [100, 300, 400, 500], 'Clock speed of the pulse blaster [MHz]')
#     ])
#
#     _PROBES = {}
#
...
class LISE607RTPulseBlaster(PulseBlaster):
    """
        This Instrument controls the SpinCore Pulseblaster used in the LISE 6th floor confocal setup II
        The internal clock speed is 500MHz
        Notice that the PB channels number labeled here is smaller than that in the MATLAB software by 1 due to the
        stupid difference between MATLAB and Python
        - ZQ 12/16/2018 5:17pm
    """
    _DEFAULT_SETTINGS = Parameter([

        Parameter('laser', [
            Parameter('channel', 0, int, 'channel to which laser is connected by AOM'),
            Parameter('status', False, bool, 'True if voltage is high to the laser, false otherwise'),
            Parameter('delay_time', 455, float,
                      'delay time between pulse sending time and laser switch on [ns] (measured as 470ns)')
        ]),
        Parameter('apd_readout', [
            Parameter('channel', 14, int, 'channel to which the counter1 daq is connected to'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('apd_switch', [
            Parameter('channel', 3, int, 'additional channel to which the counter1 daq is also connected to'),
            Parameter('status', False, bool, 'True if voltage is high to the daq, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and daq acknowledgement [ns]')
        ]),
        Parameter('microwave_i', [
            Parameter('channel', 8, int, 'channel to which the I port is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_q', [
            Parameter('channel', 9, int, 'channel to which the Q port is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_i_II', [
            Parameter('channel', 16, int, 'channel to which the Marki I port is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_q_II', [
            Parameter('channel', 17, int, 'channel to which the Marki Q port is connected'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_switch', [
            Parameter('channel', 1, int, 'channel to which the MW switch is connected to and controls the IQ signals'),
            Parameter('status', False, bool, 'True if voltage is high to the trig_channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_switch_II', [
            Parameter('channel', 4, int, 'channel to which the MW switch II is connected to. if off, RF will go to the Marki IQ mixer'),
            Parameter('status', False, bool, 'True if voltage is high to the RF switch, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and RF switch [ns]')
        ]),

        Parameter('microwave_switch_III', [
            Parameter('channel', 6, int, 'channel to which the MW switch III is connected to'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('microwave_switch_IV', [
            Parameter('channel', 12, int, 'channel to which the MW switch IV is connected to. this switch is after the Marki IQ mixer'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('fg_trigger', [
            Parameter('channel', 7, int, 'trigger the Agilent 33120A function generator'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('aux', [
            Parameter('channel', 13, int,
                      'channel to the auxiliary channel is connected to'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('aux2', [
            Parameter('channel', 18, int,
                      'channel to the auxiliary channel is connected to'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('off_channel_PB3', [
            Parameter('channel', 2, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('off_channel_PB6', [
            Parameter('channel', 5, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),

        Parameter('off_channel_PB20', [
            Parameter('channel', 19, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        Parameter('off_channel_PB21', [
            Parameter('channel', 20, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        Parameter('off_channel_PB22', [
            Parameter('channel', 21, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        Parameter('off_channel_PB23', [
            Parameter('channel', 22, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
        Parameter('off_channel_PB24', [
            Parameter('channel', 23, int, 'off-channel - nothing is connected here'),
            Parameter('status', False, bool, 'True if voltage is high to the off-channel, false otherwise'),
            Parameter('delay_time', 0, float, 'delay time between pulse sending time and off channel on [ns]')
        ]),
    ...
    def __init__(self, name=None, settings=None):
        #COMMENT_ME
        super(LISE607RTPulseBlaster, self).__init__(name, settings)

    def update(self, settings):
        # call the update_parameter_list to update the parameter list
        # oh god this is confusing
        super(LISE607RTPulseBlaster, self).update(settings)
...
if __name__ == '__main__':
    ...
    # Comment out the original codes from B26_toolkit master -Ziwei 12/15/2018
    # pb = B26PulseBlaster()
    ...
    pb.update(pb._DEFAULT_SETTINGS)



    # pb.update({'off_channel': {'channel': 1}})
    # pb.update({'off_channel': {'status': True}})



    print('off_channel is updated')
    # for i in [0 if i % 2 == 0 else 1 for i in range(5)]:
    #     pb.update({'off_channel': {'status': bool(i)}})
    #     time.sleep(.5)
