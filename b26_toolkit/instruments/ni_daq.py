...
class DAQ(Instrument):
    ...
    def setup_counter(self, channel, sample_num, continuous_acquisition=False):
        ...
        # task['timeout'] = float64(50 * (1 / task['sample_rate']) * task['sample_num']) # change timeout to be longer ZQ 2/7/2019
        task['timeout'] = float64(5 * (1 / task['sample_rate']) * task['sample_num'])
        # print('task[sample_rate]') # Is sample rate really zero?? ZQ 2/7/2019
    ...
    def setup_gated_counter(self, channel, num_samples):
        ...
        """
        if 'digital_input' not in list(self.settings.keys()):
            print('ValueError: This DAQ does not support digital input')
            raise ValueError('This DAQ does not support digital input')
        if not channel in list(self.settings['digital_input'].keys()):
            print('KeyError: This is not a valid digital input channel')
            raise KeyError('This is not a valid digital input channel')
        ...
        # channel_settings = self.settings['digital_input'][channel]
        # task['sample_rate'] = float(channel_settings['sample_rate'])
        # # added to set sample rate ZQ 2/7/2019
        #
        # task['timeout'] = float64(5 * (1 / task['sample_rate']) * task['sample_num'])
        # # added to set timeout ZQ 2/7/2019
    ...
    def read_counter(self, task_name):
        ...
        """
        # counterTimeOut = 10
        # counterTimeOut = 30 # set the timeout to be 30sec to allow longer sequence! ZQ 4/21/2019
        counterTimeOut = 200  # set the timeout to be 200sec to allow the very long T1 measurement! ZQ 6/20/2019

        # print('start read_counter')
        ...
        # print('before self._check_error9')
        # AB 08/02/2017: changed DAQmxReadCounterF64 timeout from infinity (-1) to 10s to prevent freezes
        # print(task_handle_ctr,int32(task['num_samples_per_channel']),float64(counterTimeOut),ctypes.byref(data),uInt32(task['sample_num']),ctypes.byref(samplesPerChanRead),None)
        self._check_error_and_skip(self.nidaq.DAQmxReadCounterF64(task_handle_ctr,
                                                                  int32(task['num_samples_per_channel']),
                                                                  float64(counterTimeOut),
                                                                  ctypes.byref(data),
    ...
    def setup_AO(self, channels, waveform, clk_source=""):
        ...
        if 'analog_output' not in list(self.settings.keys()):
            print('ValueError: This DAQ does not support analog output')
            raise ValueError('This DAQ does not support analog output')
        for c in channels:
            if not c in list(self.settings['analog_output'].keys()):
                print('KeyError: This is not a valid analog output channel')
        ...
        # print('1 - for c in channels:, sample rate = ')
        # print(task['sample_rate'])
        ...
        for c in channels:
            if not self.settings['analog_output'][c]['sample_rate'] == task['sample_rate']:
                ...
                print('ValueError: All sample rates must be the same')
                raise ValueError('All sample rates must be the same')
        channel_list = ''.encode('ascii')

        # print('2 - for c in channels:')
        ...
        # print('len(numpy.shape(waveform))')
        # print(len(numpy.shape(waveform)))
    ...
    def setup_AI(self, channel, num_samples_to_acquire, continuous = False, clk_source=""):
     ...
     #   print(channel_list)
        if 'analog_input' not in list(self.settings.keys()):
            print('ValueError: This DAQ does not support analog input')
    ...
    def setup_DO(self, channels):
        ...
        # print(self.settings['digital_output'].keys())
        for c in channels:
            if not c in list(self.settings['digital_output'].keys()):
                print(self.settings['digital_output'].keys())
                print(c)
                print('KeyError: This is not a valid digital output channel')
        ...
        # print('finished setting sample rate')
        ...
        for c in channels:
            ...
            lines_list += self.settings['device'] + '/port0/line' + str(
                self.settings['digital_output'][c]['channel']) + ', '
        lines_list = lines_list[:-2].encode('ascii')  # remove the last
    ...
    def DO_write(self, task_name, output_values):
        ...
        self._check_error(self.nidaq.DAQmxWriteDigitalLines(task['task_handle'], #taskHandle
                                                            int32(sample_num), #numSampsPerChan
                                                            bool32(False), #autoStart
                                                            float64(-1), #timeout
                                                            # DAQmx_Val_GroupByScanNumber,

                                                            DAQmx_Val_GroupByChannel,
                                                            	#dataLayout
                                                            np.array(output_values).ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), #writeArray
                                                            None,
                                                            None))
    ...
    def read(self, task_name):
        if 'ctr' in task_name:
            ...
            print('This task does not allow reads.')
    ...
    def write(self, task_name, output_values):
        if 'do' in task_name:
            ...
            print('This task does not allow writes.')
    ...
    def _check_error(self, err):
        ...
        """
        # print('start the function _check_error')
        # print('err is ')
        # print(err)
        if err < 0:
            buffer_size = 1000
            ...
            print('RuntimeError: nidaq call failed with error %d: %s' % (err, buffer.value))
    ...
    def _check_error_and_skip(self, err):
        ...
        This was copied from ni_daq_v2
    ...
    @classmethod
    def get_connected_devices(cls, verbose = False):
        """
...
class NI6353(DAQ):
    """
    This class implements the NI PCIe-6353 DAQ, which includes 32 AI, 4 AO, and 48 DI/DO channels and inherits basic
    input/output functionality from DAQ. A subset of these channels are accessible here, but more can be added up to
    these limits.
    currently used in LISE 6th confocal setup II
    - updated by ZQ 1/3/2019 4:38pm
    """
    _DEFAULT_SETTINGS = Parameter([
        Parameter('device', 'Dev1', ['Dev1'], 'Name of DAQ device'),
        Parameter('override_buffer_size', -1, int, 'Buffer size for manual override (unused if -1)'),
                  ...
                  [
                      Parameter('ctr0',
                                [
                                    Parameter('input_channel', 0, list(range(0, 32)), 'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 8, list(range(0, 32)), 'PFI for counter channel input'),
                                    Parameter('gate_PFI_channel', 7, list(range(0, 32)), 'PFI for counter channel input'),
                                    Parameter('clock_PFI_channel', 13, list(range(0, 32)), 'PFI for clock channel output'),
                                    Parameter('clock_counter_channel', 1, [0, 1], 'channel for clock output'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
                      ...
                      Parameter('ctr2',
                                [
                                    Parameter('input_channel', 2, list(range(0, 32)), 'channel for counter signal input'),
                                    Parameter('counter_PFI_channel', 0, list(range(0, 32)), 'PFI for counter channel input'),
                                    Parameter('gate_PFI_channel', 1, list(range(0, 32)), 'PFI for counter channel input'),
                                    Parameter('clock_PFI_channel', 13, list(range(0, 32)), 'PFI for clock channel output'),
                                    Parameter('clock_counter_channel', 1, [0, 1], 'channel for clock output'),
                                    Parameter('sample_rate', 1000.0, float, 'input sample rate (Hz)')
                                ]
        ...
        Parameter('digital_output',
                  [
                      # Parameter('do0', [Parameter('channel', 0, list(range(0, 16)), 'channel')]),
                      # Parameter('do1', [Parameter('channel', 1, list(range(0, 16)), 'channel')]),
                      # Parameter('do3', [Parameter('channel', 3, list(range(0, 16)), 'channel')]),
                      # Parameter('do4', [Parameter('channel', 4, list(range(0, 16)), 'channel')])
                      Parameter('do0',
                                [
                                    Parameter('channel', 0, list(range(0, 16)), 'channel'),
                                    # Parameter('value', False, bool, 'value')
                      ...
                      Parameter('do1',
                                [
                                    Parameter('channel', 1, list(range(0, 16)), 'channel'),
                                    # Parameter('value', False, bool, 'value')
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
                                    # Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    # Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                      ...
                      Parameter('do3',
                                [
                                    Parameter('channel', 3, list(range(0, 16)), 'channel'),
                                    # Parameter('value', False, bool, 'value')
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
                                    # Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    # Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
                      ...
                      Parameter('do4',
                                [
                                    Parameter('channel', 4, list(range(0, 16)), 'channel'),
                                    # Parameter('value', False, bool, 'value')
                                    Parameter('sample_rate', 1000.0, float, 'output sample rate (Hz)')
                                    # Parameter('min_voltage', -10.0, float, 'minimum output voltage (V)'),
                                    # Parameter('max_voltage', 10.0, float, 'maximum output voltage (V)')
