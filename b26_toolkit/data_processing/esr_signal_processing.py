...
def fit_esr(freq, ampl, min_counts = .5, contrast_factor = 1.5, num_of_peaks = -1, strain_filtering=False, verbose = True):
    """
    Returns lorentzian fit parameters for a typical NV esr sweep, giving 4 or 6 parameters depending on if 1 or 2
    lorentzian dips are detected.
    Args:
        ...
        num_of_peaks (-1, 1 or 2): specify number of peaks to be fitted. -1 means not specifying it.
    Returns:
        fit parameters for either a single or double lorentizian peak fit to the data, as figured out by find_nv_peaks.
        if a single lorentzian was fit, the list [constant_offset, amplitude, center, fwhm] is returned;
        otherwise, the list [constant_offset, amplitude1, amplitude2, center1, center2, fwhm] is returned
    ...
    if max(freq) < F0:
        ...
        try:
            ...
            if num_of_peaks != 2:
                ...
                # fit = fit_lorentzian(freq, ampl, starting_params=start_vals,
                #                      bounds=[(0, -np.inf, 0, 1e6), (np.inf, 0, np.inf, np.inf)])
        except:
            print('ESR fit failed!')
            fit = None
            return fit
    else:
        ...
        try:
            if len(freq_peaks) == 2:
                ...
                # if num_of_peaks != 1:
                    ...
                    if len(fit) == 6 and num_of_peaks == 1:
                        if verbose:
                            print('Found 2 peaks, but only need 1 peak. Return fit = none.')
                        fit = None
                        return fit
                    elif len(fit) == 4 and num_of_peaks == 2:
                        if verbose:
                            print('Found 1 peak, but need 2 peaks. Return fit = none.')
                        fit = None
                        return fit
    ...
    # Check minimum counts requirement
    if not fit is None and fit[0] < min_counts:
        print('(fit[0] < min_counts. Fit is none.)')

        fit = None
        return fit

    # Check contrast factor requirement (EXPERIMENTAL, REMOVE IF PROBLEMATIC)
    ...
    if not fit is None:
        if len(fit) == 6:
            ...
            if calc_esr_noise(freq, ampl, fit) * contrast_factor > np.abs(fit[1]):
                fit = None
        if fit is None:
            print('(contrast factor requirement is not satisfied. Fit is none.)')
