...
#===============================================================
def get_lorentzian_fit_starting_values(x_values, y_values, negative_peak=True, verbose = False):
    """
    ...
    constant_offset = round(float(np.mean(y_values)*1.03), 2)
    amplitude = min(y_values) - max(y_values) + 0.5 * np.std(y_values)  # min - max overestimates the amplitude, that's why we reduce the amplitude by the stddev
    if negative_peak is False:
        amplitude = -amplitude
    amplitude = round(amplitude,2)
    #center = np.mean(x_values) # assume the resonance is near the center
    center = x_values[np.argmin(y_values)]
    ...
    fwhm = .012e9
    if verbose:
        print('fit starting values:',[constant_offset, amplitude, center, fwhm])
...
def fit_lorentzian(x_values, y_values, starting_params=None, bounds=None):
    ...
    if bounds:
        return optimize.curve_fit(lorentzian, x_values, y_values, p0=starting_params, bounds=bounds, max_nfev=5000)[0]
    else:
...
def fit_rabi_decay(t, y, variable_phase=True, verbose=True, return_guess = False):
    """
    ...
    # opt = optimize.minimize(cost_function_fit, np.asarray(initial_parameter))
    opt = optimize.minimize(cost_function_fit, np.asarray(initial_parameter),
                            bounds=[(None, None), (0.1 * wx, 10 * wx), (None, None), (None, None), (None, None)])

    #
    # [ax, wx, phi, offset, tau] = opt.x

    if verbose:
        print(('optimization result:', opt.x))
        # print(('optimization result:', opt))
    if return_guess:
